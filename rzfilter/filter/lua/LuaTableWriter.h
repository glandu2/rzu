...
public:
	...
	//Primitives
	template<typename T>
	typename std::enable_if<is_primitive<T>::value && !std::is_floating_point<T>::value, void>::type
	...
	//Boolean
	template<typename T>
	void read(const char* fieldName, bool& val) {
		luaGet(fieldName);

		if(!lua_isinteger(L, -1)) {
			Object::logStatic(Object::LL_Error, "rzfilter_lua_module", "Bad field type for %s, expected integer, got %s\n", fieldName, lua_typename(L, lua_type(L, -1)));
			ok = false;
		}
	...
	template<typename T, typename U>
	typename std::enable_if<is_primitive<U>::value && !std::is_floating_point<U>::value && !std::is_same<U, bool>::value, void>::type
	...
	template<typename T, typename U>
	typename std::enable_if<is_primitive<U>::value && std::is_floating_point<U>::value, void>::type
	read(const char* fieldName, U& val) {
		luaGet(fieldName);

		if(!lua_isnumber(L, -1)) {
			Object::logStatic(Object::LL_Error, "rzfilter_lua_module", "Bad field type for %s, expected number, got %s\n", fieldName, lua_typename(L, lua_type(L, -1)));
			ok = false;
		}
	...
	read(const char* fieldName, T& val) {
		...
		if(!lua_istable(L, -1)) {
			Object::logStatic(Object::LL_Error, "rzfilter_lua_module", "Bad field type for %s, expected table, got %s\n", fieldName, lua_typename(L, lua_type(L, -1)));
			ok = false;
		} else {
	...
	void readString(const char* fieldName, std::string& val, size_t size) {
		...
		if(!lua_isstring(L, -1)) {
			Object::logStatic(Object::LL_Error, "rzfilter_lua_module", "Bad field type for %s, expected string, got %s\n", fieldName, lua_typename(L, lua_type(L, -1)));
			ok = false;
		}
	...
	//All fixed arrays
	template<typename T, typename U>
	void readArray(const char* fieldName, U* val, size_t size) {
		luaGet(fieldName);

		if(!lua_istable(L, -1)) {
			Object::logStatic(Object::LL_Error, "rzfilter_lua_module", "Bad field type for %s, expected array, got %s\n", fieldName, lua_typename(L, lua_type(L, -1)));
			ok = false;
		} else {
	...
	//Dynamic array
	template<typename T>
	void readDynArray(const char* fieldName, std::vector<T>& val, uint32_t sizeToRead) {
		luaGet(fieldName);
