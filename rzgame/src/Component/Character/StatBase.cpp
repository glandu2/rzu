	...
	for(size_t i = 0; i < size; i++) {
		...
		if(jobBonus == nullptr) {
			Object::logStatic(Object::LL_Error,
			                  "StatBase",
			                  "Couldn't compute stat bonus for jobid %d, job not found in JobLevelBonus\n",
			                  jobs[i]);
			continue;
		}
		...
		this->base.dexterity +=
		    computeSumProduct(jobBonus->dexterity, levelParts, partNum) + jobBonus->default_dexterity;
		this->base.agility += computeSumProduct(jobBonus->agility, levelParts, partNum) + jobBonus->default_agility;
		this->base.intelligence +=
		    computeSumProduct(jobBonus->intelligence, levelParts, partNum) + jobBonus->default_intelligence;
