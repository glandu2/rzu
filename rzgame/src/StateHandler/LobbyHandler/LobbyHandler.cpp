...
void LobbyHandler::onCharacterWearInfoResult(DbQueryJob<CharacterWearInfoBinding>* query) {
	...
	for(; it != itEnd; ++it) {
		...
		for(; wearIt != wearItEnd; ++wearIt) {
			...
			if(dbCharacterInfo->sid == dbWearInfo->character_sid && dbWearInfo->wear_info >= 0 &&
			   (size_t) dbWearInfo->wear_info < sizeof(characterInfo.wear_info) / sizeof(characterInfo.wear_info[0])) {
	...
	log(LL_Debug,
	    "Account %s has %d characters\n",
	    session->getAccount().c_str(),
	    (int) characterList.characters.size());
}
...
void LobbyHandler::onCheckCharacterName(const TS_CS_CHECK_CHARACTER_NAME* packet) {
	...
	if(name.size() > 18 || name.size() < 4) {
		...
		lobbyQueries.executeDbQuery<CheckCharacterNameBinding>(
		    this, &LobbyHandler::onCheckCharacterNameExistsResult, input);
	}
}
...
void LobbyHandler::onCreateCharacter(const TS_CS_CREATE_CHARACTER* packet) {
	...
	if(packet->character.name != lastValidatedCharacterName) {
		log(LL_Warning,
		    "Client tried to create character without prior check for name: %s\n",
		...
		log(LL_Info,
		    "Account %s sent create character without having retrieved character list\n",
		    session->getAccount().c_str());
		session->sendResult(packet->packetID, TS_RESULT_NOT_ACTABLE, 0);
...
void LobbyHandler::onCreateCharacterResult(DbQueryJob<CreateCharacterBinding>* query) {
	if(query->getInput()->out_character_sid == 0) {
		log(LL_Warning,
		    "Create character \"%s\" failed for account %s\n",
		    query->getInput()->name.c_str(),
		    query->getInput()->account_name);
		session->sendResult(TS_CS_CREATE_CHARACTER::packetID, TS_RESULT_DB_ERROR, 0);
	} else {
		log(LL_Debug,
		    "Created character \"%s\" for account %s with sid %" PRId64 "\n",
		    query->getInput()->name.c_str(),
		    query->getInput()->account_name,
		    query->getInput()->out_character_sid);
...
void LobbyHandler::onDeleteCharacter(const TS_CS_DELETE_CHARACTER* packet) {
	...
	if(!hasCharacter) {
		...
		log(LL_Info,
		    "Account %s tried to delete non existing character %s\n",
		    session->getAccount().c_str(),
		    name.c_str());
	} else {
...
void LobbyHandler::onCharacterLogin(const TS_CS_LOGIN* packet) {
	...
	if(Character::getObjectBySid(selectCharacter.get()->sid) == nullptr) {
		...
		log(LL_Warning,
		    "Character %s of account %s is already logged in\n",
		    selectCharacter.get()->name.c_str(),
		    session->getAccount().c_str());
