...
template<class T> static void initMaxSidBinding(DbConnectionPool* dbConnectionPool, const char* query) {
	DbQueryJob<T>::createBinding(
	    dbConnectionPool, CONFIG_GET()->game.telecaster.connectionString, query, DbQueryBinding::EM_OneRow);
...
template<> void DbQueryJob<GameServer::AwakenOptionSidBinding>::init(DbConnectionPool* dbConnectionPool) {
	initMaxSidBinding<GameServer::AwakenOptionSidBinding>(dbConnectionPool,
	                                                      "select max(sid) as max_sid from AwakenOption");
}
...
template<> void DbQueryJob<GameServer::TitleConditionSidBinding>::init(DbConnectionPool* dbConnectionPool) {
	initMaxSidBinding<GameServer::TitleConditionSidBinding>(dbConnectionPool,
	                                                        "select max(sid) as max_sid from TitleCondition");
}
