...
void GameServerSession::onLogin(const TS_SU_LOGIN* packet) {
	...
	if(!IconServerSession::checkName(serverName.c_str(), serverName.size())) {
		...
		log(LL_Error,
		    "Server name (app.name in gameserver.opt) has invalid characters, only these are allowed: %s\n",
		    IconServerSession::getAllowedCharsForName());
		result.result = TS_RESULT_INVALID_TEXT;
	} else {
		std::pair<ServerIterator, bool> insertResult =
		    servers.insert(std::pair<std::string, GameServerSession*>(serverName, this));
...
void GameServerSession::onRequestUpload(const TS_SU_REQUEST_UPLOAD* packet) {
	...
	log(LL_Debug,
	    "Upload request for client %u with account id %u for guild %u\n",
	    packet->client_id,
	    packet->account_id,
	    packet->guild_sid);

	UploadRequest::pushRequest(
	    this, packet->client_id, packet->account_id, packet->guild_sid, packet->one_time_password);
