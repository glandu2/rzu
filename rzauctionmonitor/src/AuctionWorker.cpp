...
AuctionWorker::AuctionWorker(AuctionManager *auctionManager,
	...
	: auctionManager(auctionManager),
      ...
      authSession(&gameSession, ip, port, serverIdx, delayTime, useRsa, account, password, CONFIG_GET()->client.version),
	  isConnected(false),
	  idle(true)
{
...
bool AuctionWorker::doAuctionSearch()
{
	if(!request && isConnected) {
		...
		if(request) {
			...
			auctionSearchTimer.start(this, &AuctionWorker::onAuctionSearchTimer, CONFIG_GET()->client.auctionSearchTimeout.get(), 0);
			return true;
		} else {
			return false;
		}
...
void AuctionWorker::onAuctionSearchResult(const TS_SC_AUCTION_SEARCH *packet)
{
	...
	if(request) {
		...
		for(int i = 0; i < packet->auction_info_count; i++) {
			...
			auctionManager->addAuctionInfo(request.get(), auctionInfo->uid, (const uint8_t*)auctionInfo, auctionInfoSize);
		}
		log(LL_Info, "Auction search of category %d, page %d/%d found %d results\n", request->category, request->page, packet->total_page_count, packet->auction_info_count);
		auctionManager->onAuctionSearchCompleted(true, packet->total_page_count, std::move(request));
	} else {
		log(LL_Error, "Received auction results without request: page: %d/%d, %d items\n", packet->page_num, packet->total_page_count, packet->auction_info_count);
	}
}
...
void AuctionWorker::onAuctionSearchFailed(int resultCode)
{
	...
	if(request) {
		log(LL_Warning, "Auction search of category %d, page %d failed with code %d\n", request->category, request->page, resultCode);
