...
template<typename D, typename Key, typename Container, typename Compare, typename = void>
...
public:
	...
	mapped_type& operator[](key_type const& key) { return self()->try_emplace(key).first.underlying->second; }

	mapped_type& operator[](key_type&& key) { return self()->try_emplace(std::move(key)).first.underlying->second; }

	// Modifiers

	std::pair<iterator, bool> insert(value_type const& value) { return insert_(value); }

	std::pair<iterator, bool> insert(value_type&& value) { return insert_(std::move(value)); }

	template<class InputIt> void insert(InputIt first, InputIt last, delay_sort_t) {
		this->ds_insert_(first, last);
		auto it =
		    std::unique(self()->container.begin(), self()->container.end(), impl::eq_comp<value_compare>{value_comp()});
	...
	template<typename M> std::pair<iterator, bool> insert_or_assign(const_iterator hint, key_type const& key, M&& obj) {
		return insert_or_assign(key, std::forward<M>(obj));
	}

	template<typename M> std::pair<iterator, bool> insert_or_assign(const_iterator hint, key_type&& key, M&& obj) {
	...
	template<typename... Args> iterator try_emplace(const_iterator hint, key_type const& key, Args&&... args) {
	...
	size_type count(key_type const& key) const { return self()->find(key) != self()->end(); }

private:
	template<typename V> std::pair<iterator, bool> insert_(V&& value) {
		iterator it = self()->lower_bound(value.first);
		if(it == self()->end() || self()->value_comp()(value, *it)) {
			it = self()->container.insert(it.underlying, std::forward<V>(value));
			return std::make_pair(it, true);
		}
		return std::make_pair(it, false);
	}

	template<typename K, typename M> std::pair<iterator, bool> insert_or_assign_(K&& key, M&& obj) {
		iterator it = self()->lower_bound(key);
		if(it == self()->end() || self()->key_comp()(key, it->first)) {
			it = self()->container.insert(it.underlying, value_type(std::forward<K>(key), std::forward<M>(obj)));
			return std::make_pair(it, true);
		}
	...
	template<typename K, typename... Args> std::pair<iterator, bool> try_emplace_(K&& key, Args&&... args) {
		iterator it = self()->lower_bound(key);
		if(it == self()->end() || self()->key_comp()(key, it->first)) {
...
class flat_map_base<D, Key, Container, Compare, std::void_t<typename Compare::is_transparent>>
...
public:
	...
	template<class P> std::pair<iterator, bool> insert(P&& value) {
		iterator it = self()->lower_bound(value.first);
		if(it == self()->end() || self()->value_comp()(value, *it)) {
			it = self()->container.insert(it.underlying, std::forward<P>(value));
			return std::make_pair(it, true);
		}
		return std::make_pair(it, false);
	}
	...
	template<typename K> size_type count(K const& key) const { return self()->find(key) != self()->end(); }
};
...
class flat_map
    : public impl::
          flat_map_base<flat_map<Container, Compare>, typename Container::value_type::first_type, Container, Compare> {
