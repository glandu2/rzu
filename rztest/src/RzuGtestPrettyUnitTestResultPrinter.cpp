...
void RzuTestPrinter::OnTestIterationStart(
	...
	log(LL_Info, "[==========] Running %d tests from %d test cases.\n",
...
void RzuTestPrinter::OnTestCaseEnd(const testing::TestCase& test_case) {
	...
	log(LL_Info, "[----------] %d tests from %s (%d ms total)\n",
...
void RzuTestPrinter::OnTestIterationEnd(const testing::UnitTest& unit_test,
										int /*iteration*/) {
	if (testing::GTEST_FLAG(print_time)) {
		log(LL_Info, "[==========] %d tests from %d test cases ran. (%d ms total)\n",
			 unit_test.test_to_run_count(),
			 unit_test.test_case_to_run_count(),
			 (int)unit_test.elapsed_time());
	} else {
		log(LL_Info, "[==========] %d tests from %d test cases ran.", unit_test.test_to_run_count(), unit_test.test_case_to_run_count());
	}
